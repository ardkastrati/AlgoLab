	...
	for(int i = 0; i < n; i++) {
		for(int j = 0; j < m; j++) {
			...
			if(dist1 > dist2) {
				...
				if(free_nails[i].first <= occupied_nails[j].first) {
					lp.set_a(i, k, w); lp.set_b(k, 2*(occupied_nails[j].first - free_nails[i].first) - w);
				} else {
					lp.set_a(i, k, w); lp.set_b(k, -2*(occupied_nails[j].first - free_nails[i].first) - w);
				}
			}  else {
				if(free_nails[i].second <= occupied_nails[j].second) {
					lp.set_a(i, k, h); lp.set_b(k, 2*(occupied_nails[j].second - free_nails[i].second) - h);
				} else {
					lp.set_a(i, k, h); lp.set_b(k, -2*(occupied_nails[j].second - free_nails[i].second) - h);
				}
			}
