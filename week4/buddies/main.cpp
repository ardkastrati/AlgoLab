...
typedef std::vector<std::set<int> > VSetI;
...
void testcases() {
	...
	for(int i = 0; i < n; i++) {
		for(int j = 0; j < c; j++) {
			...
			if(word_index.find(curr_characteristic) == word_index.end()) {
				word_index[curr_characteristic] = word.size();
				word.push_back(curr_characteristic);
			}
